...
public class GTGDMultiplayerScript : MonoBehaviour {
	
	//Variables Start___________________________________
	
	private string titleMessage = "Choose or create your own server";
	...
	private int serverDisWindowWidth = 220;
	
	private int serverDisWindowHeight = 40;
	
	private int serverDisWindowLeftIndent = 200;
	
	private int serverDisWindowTopIndent = 0;
	...
	private string gameNameType = "Blood Key ver1.0";
	...
	private HostData[] hostDataBuffer;
	...
	private int newport=0;
		...
		MasterServer.ipAddress = "134.0.112.68";
		MasterServer.port = 23466;
        Network.natFacilitatorIP = "134.0.112.68";
	...
	void ConnectWindow(int windowID)
	{
			...
			iWantToConnectToAPublicServer == false)
		{
			...
			{
				if(GUILayout.Button("Exit Game", GUILayout.Height(btnHeightCntWindow)))
				{
					Application.Quit();	
				}
			}
		...
		{	
			...
			{				
				...
				Network.InitializeServer(numberOfPlayers,connectionPort+newport,!Network.HavePublicAddress());	
		        newport++;
				
				MasterServer.RegisterHost(gameNameType, serverName, "open");
		...
		if(iWantToSetupAPrivateServer == true)
		{
			...
			if(GUILayout.Button("Start server", GUILayout.Height(btnHeightCntWindow/2)))
			{
				//Create the server
		...
		{	
			...
			{				
				...
				{	if(hostData[i].connectedPlayers==numberOfPlayers+1)
						continue;
					GUILayout.BeginHorizontal();
						...
						GUILayout.Height(btnHeightCntWindow/2), GUILayout.Width(sbWidth)))
					{
						...
						if(playerName != "")
						{
							...
							//если игроков больше 3, то отмечаем серевер как закрытый(не работает)
							//if(hostData[i].connectedPlayers>2)
								//MasterServer.RegisterHost(gameNameType,hostData[i].gameName, "Closed");
						}		
					}
					
					//Dispaly the number of players currently in the server and the max number of players.
				
					GUILayout.Label((hostData[i].connectedPlayers ) + "/" + (hostData[i].playerLimit ), boldStyleCentered, 
	...
	{	
		...
		//находим все закрытые сервера и не включаем их в рабочий hostData[]		
		/*
		hostDataBuffer = MasterServer.PollHostList();
		int openServerCounter=hostDataBuffer.Length;
		for(int i = 0; i < hostDataBuffer.Length; i++){
			if(hostDataBuffer[i].comment.Equals("Closed"))
			 	openServerCounter--;
		}
		hostData = new HostData[openServerCounter];
		for(int i = 0; i < hostDataBuffer.Length; i++){
			if(hostDataBuffer[i].comment.Equals("Closed"))
				continue;
			hostData[i]=hostDataBuffer[i];
		}*/
	...
	void ClientDisconnectWindow(int windowID)
	{
		...
		if(GUILayout.Button("Options", GUILayout.Height(btnHeightSmaller)))
		{
			/*GameObject gManager = GameObject.Find("GameManager");
			
			GameSettings settingsScript = gManager.GetComponent<GameSettings>();
			
			settingsScript.volumeSlider = settingsScript.volume;
			
			settingsScript.mouseSlider = settingsScript.mouseSensitivity;
